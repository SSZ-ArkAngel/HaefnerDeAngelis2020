...
organized_data %>%
  ...
  ggplot(aes(x = as.numeric(apertureDegree), y = relativeTilt)) +
  ...
  stat_summary(
    ...
    mapping = aes(fill = conditionName), fun.data = mean_cl_normal, geom = "ribbon", alpha = 0.1, show.legend = FALSE) +
  stat_summary(
    data = . %>%
      filter(!(conditionName %in% c("Global", "Local"))) %>%
      group_by(subject, conditionName) %>%
      mutate(relativeTilt = mean(relativeTilt)), 
    mapping = aes(group = conditionName), fun.data = mean_cl_normal, geom = "line", linetype = "dashed", alpha = 0.3, show.legend = FALSE) +
