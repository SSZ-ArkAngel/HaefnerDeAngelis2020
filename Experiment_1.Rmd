...
files = list.files(path = "/Users/user/R-workspace", pattern = "*.csv", full.names = T)
...
organized_data %>%
  ...
  ggplot(aes(x = as.numeric(apertureDegree), y = relativeTilt, shape = conditionName)) + # color: to maintain coherence in legend
  ...
  stat_summary(
    ...
    mapping = aes(fill = conditionName), fun.data = mean_cl_normal, geom = "ribbon", alpha = 0.1, show.legend = FALSE) + # or mean_cl_boot
  stat_summary(
    data = . %>% # . indicates the data frame
      filter(!(conditionName %in% c("Global", "Local"))) %>%
      group_by(subject, conditionName) %>%
      mutate(relativeTilt = mean(relativeTilt)), 
    mapping = aes(group = conditionName), fun.data = mean_cl_normal, geom = "line", linetype = "dashed", alpha = 0.3, show.legend = FALSE) + # or mean_cl_boot
  
  stat_summary(
    data = . %>% # . indicates the data frame
      filter(conditionName %in% c("Global", "Local")), 
    mapping = aes(color = conditionName), fun.data = mean_cl_normal, geom = "pointrange", position = position_dodge(0.1)) + # or mean_cl_boot
  stat_summary(
    data = . %>% # . indicates the data frame
      filter(conditionName %in% c("Global", "Local")), 
    mapping = aes(color = conditionName), fun.data = mean_cl_normal, geom = "line", position = position_dodge(0.1)) + # or mean_cl_boot
  
  scale_color_manual(
    name = "Condition",
    breaks = levels(organized_data$conditionName),
    values = c("grey40", "green4", "chocolate1", "dodgerblue2"), # CHANGE!!!!! gray?
    drop = FALSE) +
  scale_fill_manual(
    breaks = levels(organized_data$conditionName),
    values = c("grey40", "green4", "chocolate1", "dodgerblue2"), # CHANGE!!!!! gray?
...
## Experiment 1 every individual with stat_summary normal CIs
```{r Exp 1 four subjects, fig.width=6}
organized_data %>%
  # Select experiment 1 data
  filter(experiment == "1") %>%
  
  # Build plot
  ggplot(aes(x = as.numeric(apertureDegree), y = relativeTilt, color = conditionName, 
  ...
  stat_summary(
    data = . %>%
      ...
      group_by(subject, conditionName) %>%
      mutate(relativeTilt = mean(relativeTilt)), 
    mapping = aes(fill = conditionName), fun.data = mean_cl_normal, geom = "pointrange", show.legend = FALSE) +
  stat_summary(
    data = . %>%
      filter(!(conditionName %in% c("Global", "Local"))) %>%
      group_by(subject, conditionName) %>%
      mutate(relativeTilt = mean(relativeTilt)), 
    mapping = aes(fill = conditionName), fun.data = mean_cl_normal, geom = "line", show.legend = FALSE) +
  
  stat_summary(
    data = . %>%
      filter(conditionName %in% c("Global", "Local")), 
    mapping = aes(color = conditionName), fun.data = mean_cl_normal, geom = "pointrange", position = position_dodge(0.1)) +
  stat_summary(
    data = . %>%
      filter(conditionName %in% c("Global", "Local")), 
    mapping = aes(color = conditionName), fun.data = mean_cl_normal, geom = "line", position = position_dodge(0.1)) +
  ...
  # Add title
  ggtitle("Relative Tilt versus Aperture Size", "4 subjects in experiment 1")  +
  scale_x_continuous(breaks=c(1,1.5, 2, 4)) +
  theme(legend.title = element_blank(),
